        ...
        Ok(new_config) => {
			...
			let _ = app_handle.manager_mut(|man| {
				man.add_network_to_config(
					...
					NetworkData { subnet: "192.168.1.0/29".to_string(), gateway: "192.168.1.1".to_string(), dns_endpoint: Some("192.168.1.3".to_string())},
					&app_handle,
				)
			});
...
// criar NETWORK só do DNS, chamar 'add_network_to_config' após criação da config, associar logo o DNS 192.168.1.0/30 , IP 192.168.1.2. feito em src-tauri/src/main.rs line 40
#[tauri::command]
